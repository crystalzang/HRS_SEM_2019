...
#rename
setnames(rand_2014 ,
         old=c("R12ADLA", "S12ADLA", "R12CESD", "S12CESD", "R12OOPMD", "S12OOPMD", "RAGENDER", "HACOHORT"),
         new = c("R_ADL", "S_ADL", "R_CESD", "S_CESD", "R_expense", "S_expense", "sex", "cohort"))
...
id_valid_dyad_id <- id_group%>%
  ...
  mutate(dyad = "Y")
#Cleaned dyad data
dyad_helper_f_final <- left_join(rand_2014_R_valid, id_valid_dyad_id, by="HHID")%>%
  filter(dyad == "Y")
...
nrow(dyad_helper_f_final)

write.csv(dyad_helper_f_final, file="dyad_helper_f.csv", row.names = F)
```
...
#Cleaned dyad data
dyad_helper_m_final <- left_join(rand_2014_R_valid, id_valid_dyad_id, by="HHID")%>%
  filter(dyad == "Y")
...
#table(dyad_helper_f_final$sex)
nrow(dyad_helper_m_final)



write.csv(dyad_helper_m_final, file="dyad_helper_m.csv", row.names = F)
